...
let QuestionConfigService = class QuestionConfigService {
    ...
    async getAllList() {
        const questionList = await this.questionConfigRepository
            .createQueryBuilder('question_config')
            .getMany();
        return questionList;
    }
    async getAll() {
    ...
    async create(body) {
        ...
        const rule = await this.questionConfigRepository
            .createQueryBuilder('QuestionConfig')
            .select('QuestionConfig')
            .where('QuestionConfig.questionConfigTitle = :title', { title })
            .getOne();
        if (rule) {
...
QuestionConfigService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(question_config_entity_1.QuestionConfig)),
