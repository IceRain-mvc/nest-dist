"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SendEmailController = void 0;
const openapi = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
const send_email_service_1 = require("./send-email.service");
let SendEmailController = class SendEmailController {
    constructor(sendEmailService) {
        this.sendEmailService = sendEmailService;
    }
    sendEmailCode(data) {
        const { studentsList, textBefore, coder, textAfter } = data;
        return this.sendEmailService.sendEmailCode(studentsList, textBefore, coder, textAfter);
    }
    async sendFindBackPwd(params) {
        return this.sendEmailService.sendFindBackPwd(params);
    }
};
__decorate([
    (0, common_1.Post)('/sendCode'),
    openapi.ApiResponse({ status: 201, type: String }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SendEmailController.prototype, "sendEmailCode", null);
__decorate([
    (0, common_1.Post)('/sendFindBackPwd'),
    openapi.ApiResponse({ status: 201, type: String }),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SendEmailController.prototype, "sendFindBackPwd", null);
SendEmailController = __decorate([
    (0, common_1.Controller)('send-email'),
    __metadata("design:paramtypes", [send_email_service_1.SendEmailService])
], SendEmailController);
exports.SendEmailController = SendEmailController;
//# sourceMappingURL=send-email.controller.js.map